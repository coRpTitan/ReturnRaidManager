...
function ReturnRaidManager:CreateNameBox(idx)
    ...
    local y = -70 - math.fmod(idx - 1, 5) * 25
    ...
    editbox:SetScript("OnTabPressed", function() ReturnRaidManager.UI["NameBox" .. (math.fmod(idx, 40) + 1)]:SetFocus() end)

    if math.fmod(idx - 1, 5) == 0 then
...
function ReturnRaidManager:CreateSavedLayoutButton(name, idx)
    ...
    if not button then
        ...
        local y = -210 + math.fmod(idx, 4) * -25
...
function ReturnRaidManager:CreateSavedLayoutButtons()
    ...
    local names = {}
    for name, layout in pairs(self.db.global.SavedLayouts) do
        if layout then
...
function ReturnRaidManager:ToggleUI()
    ...
    if not ReturnRaidManager.UI then
        ...
        --ReturnRaidManager.UI:SetScript("OnMouseDown", function() self:StartMoving() end)
        --ReturnRaidManager.UI:SetScript("OnMouseUp", function() self:StopMovingOrSizing() end)
...
function ReturnRaidManager:LoadSavedRaidLayout(name)
    ...
    layout = self.db.global.SavedLayouts[name]
...
function ReturnRaidManager:RemoveSavedRaidLayout(name)
    if  self.db.global.SavedLayouts[name] then 
        self.db.global.SavedLayouts[name] = nil
        self:CreateSavedLayoutButtons()
    end
end
...
function ReturnRaidManager:SaveCurrentRaidLayout(name)
    ...
    local isNew = self.db.global.SavedLayouts[name] == nil

    self.db.global.SavedLayouts[name] = layout
    ReturnRaidManager.UI.LoadedRaidLayout = name
...
function ReturnRaidManager:ImportLayout()
    ...
    for i, w in words do 
        local idx = math.fmod(i - 1, 8) * 5 + floor((i - 1) / 8) + 1
